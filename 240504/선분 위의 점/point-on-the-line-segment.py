...
def check_is_not(target_arr, left, right) :
    check = False

    for i in range(n) :
        if left <= target_arr[i] <= right :
            check = True
    return check
...
for _ in range(m) : # m번 선분의 범위가 주어짐
    ...
    # 각 범위(spectrum_arr)에 좌표(target_arr)가 있는지 없는지 체크
    check = check_is_not(target_arr, left, right)
    ...
    if check : # 좌표(target_arr)가 범위(spectrum_arr) 안에 있는 경우 
        ...
        if temp_cnt == 1 : 
            ...
            for i in range(n) :
                ...
                if binary_search(target_num, left, right) : # 스펙트럼(spectrum_arr)에 target_num가 있는 경우
                    cnt += 1
